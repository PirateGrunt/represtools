...
* The only form of data exchange from one step to the next is .RDA.
...
Assume that I'm analyzing insurance performance. I'd like to 
...
Or maybe, I'd like a combination of sources.

```{r eval=FALSE}
represtools::Gather("Hitter", source = c("HTTP", "SQL"))
```
...
```{r eval=FALSE}
represtools::NewAnalysis("baseball", data="Hitter")
# Write some code
represtools::Make()

represtools::AddData("Salary")
# write some code
represtools::Clean()
represtools::Make()
